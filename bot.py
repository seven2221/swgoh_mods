...
def choose_param2(message):
    ...
    param3_options = ['Здоровье', 'Оборона', 'Критический урон', 'Критшанс', '%Стойкости', 'Атака', '%Эффективности', '%Скорости']
    for option in param3_options:
        markup3.add(option)
    bot.send_message(message.chat.id, "Выберите сет модуля:", reply_markup=markup3)

@bot.message_handler(func=lambda message: message.text in ['Здоровье', 'Оборона', 'Критический урон', 'Критшанс', '%Стойкости', 'Атака', '%Эффективности', '%Скорости'])
def choose_param3(message):
    ...
    query_param3 = {'атака': 2, '%эффективности': 7, '%скорости': 4, 'здоровье': 1, '%стойкости': 8, 'критшанс': 5,
                    'критический урон': 6, 'оборона': 3}
